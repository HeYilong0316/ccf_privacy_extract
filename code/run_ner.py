...
def conver_entity_list_to_tuple(reader, preds_list, words_list, data_dir):
    ...
    for predict_data in predict_datas:
        ...
        # 加入规则抽取的实体
        if USE_RULE:
            from ruler.regex_extractor import vx_extractor, qq_extractor
            txt_file = os.path.join(data_dir, f"{file_id}.txt")
            with open(txt_file, "r", encoding="utf8") as r:
                text = r.read()
            # (start, end, tag)
            vx_entities = vx_extractor.extract(text)
            vx_entities = [(file_id, start, end, tag) for start, end, tag in vx_entities]
            pre_entity_tuple.extend(vx_entities)
            qq_entities = qq_extractor.extract(text)
            qq_entities = [(file_id, start, end, tag) for start, end, tag in qq_entities]
            pre_entity_tuple.extend(qq_entities)

        pre_entity_tuples.extend(pre_entity_tuple) 
        
        
        ann_file = os.path.join(data_dir, f"{file_id}.csv")
        if os.path.exists(ann_file):
            ann_df = pd.read_csv(ann_file)
            for i, row in ann_df.iterrows():
                tag, start, end = row.Category, row.Pos_b, row.Pos_e
                real_entity_tuples.append((file_id, int(start), int(end)+1, tag))
