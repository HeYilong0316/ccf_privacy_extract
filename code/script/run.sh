# base
export DATA_DIR=../user_data/data/k_fold_6_510
...
for i in `seq 0 0`
do
    sh $RUN_TRAIN_FILE $DATA_DIR/"fold_"$i 64 100 -1 $OUTPUT_DIR/$i"_fold" $DATA_DIR
...
# for i in `seq 0 9`
# do
#     sh $RUN_TRAIN_FILE $DATA_DIR/"fold_"$i 32 100 -1 $OUTPUT_DIR/$i"_fold" $DATA_DIR
#     # sh $RUN_PREDICT_FILE $OUTPUT_DIR/$i"_fold"/ $DATA_DIR
#     # rm $OUTPUT_DIR/$i"_fold"/pytorch_model.bin
# done
# # layer+idcnn+crf
