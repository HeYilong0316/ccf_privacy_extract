...
class BertLayerFusion(nn.Module):
    """自动学习bert每一层的权重"""
    ...
    def forward(self, layer_sequence_hidden):
        # (batch, layer, seq, hidden)
        layer_score = []
        layer_sequence_hidden = layer_sequence_hidden[1:]
        for i, layer_output in enumerate(layer_sequence_hidden):
            layer_score.append(self.layer_weight[i](layer_output))
        # (batch, seq, layer)
        layer_score = torch.cat(layer_score, dim=2)
        # (batch, seq, layer, 1)
        layer_attention = nn.Softmax(dim=2)(layer_score).unsqueeze(-1)
        # (batch, seq, layer, hidden)
        layer_sequence_output = torch.stack(layer_sequence_hidden, dim=2)
...
class IDCNN(nn.Module):
    ...
    def __init__(self, config, hidden_size):
        ...
        for dilation in self.dilations:
            ...
            id_cnn = nn.Conv1d(in_channels=self.dense_size,
                                out_channels=self.dense_size,
                                kernel_size=self.filter_width,
                                padding=padding,
                                dilation=dilation
                                )
            id_cnn_layer.append(id_cnn)
        self.id_cnns = nn.ModuleList(id_cnn_layer)
    ...
    def forward(self, inputs):
        ...
        id_cnn_inputs = self.cnn(inputs)

        id_cnn_outputs = []
        for i in range(self.repeat_times):
            for j in range(len(self.dilations)):
                id_cnn_inputs = self.id_cnns[j](id_cnn_inputs)
                id_cnn_inputs = self.relu(id_cnn_inputs)
            id_cnn_outputs.append(id_cnn_inputs)
        # (batch, seq, hidden)
        id_cnn_outputs = torch.cat(id_cnn_outputs, dim=1)
...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config, training_args=None):
        ...
        self.classifier = nn.Linear(self.classifier_size, config.num_labels)
    ...
    def forward(
        ...
        **kwargs
        ...
        layer_hidden_states = output_hidden_states
        if self.layer_fusion:
            layer_hidden_states = True
        ...
        if self.layer_fusion:
            layer_sequence_hidden = outputs[2]
            sequence_output = self.layer_fusion(layer_sequence_hidden)
        ...
        if labels is not None:
            if not self.crf:
                ...
                if attention_mask is not None:
                    ...
                    # mask = mask.view_as(labels)
                    loss = -self.crf(logits, labels, mask=mask,
                                     reduction="mean")  # [batch_size,]
                else:
                    loss = -self.crf(logits, labels, reduction="mean")

        if not return_dict:
            output = (logits,) + outputs[2:]
            # (loss, logit, hidden_state, attention)
            return ((loss,) + output) if loss is not None else output
